...
grunt.initConfig({
  concat: {
    ...
    dist: {
      src: ['intro.js', 'project.js', 'outro.js'],
      dest: 'dist/built.js',
    },
  },
});
...
In this example, running `grunt concat:dist` will first strip any preexisting banner comment from the `project.js` file, then concatenate the result with a newly-generated banner comment, writing the output to `dist/built.js`.
...
grunt.initConfig({
  concat: {
    basic_and_extras: {
      files: {
        'dist/basic.js': ['main.js'],
        'dist/with_extras.js': ['main.js', 'extras.js'],
      },
    },
  },
});
